...
var (
	...
	myJwtMiddleware = jwtmiddleware.New(jwtmiddleware.Config{
		ValidationKeyGetter: func(token *jwt.Token) (interface{}, error) {
			...
			logger.AppendDebug("GetUserByToken", token)
...
func GetJwtMiddleware() *jwtmiddleware.Middleware {
	return myJwtMiddleware
}

func GetTokenByUser(user *Model.User) string {
	// 生成加密串过程
...
func GetUserByToken(ctx iris.Context) *Model.User {
	userToken := myJwtMiddleware.Get(ctx)
	var uid uint = 0
	if claims, ok := userToken.Claims.(jwt.MapClaims); ok && userToken.Valid {
		_id := claims["id"]
		if reflect.TypeOf(_id).Kind() == reflect.Float64 {
			uid = uint(_id.(float64))
		} else if reflect.TypeOf(_id).Kind() != reflect.String {
			id, err := strconv.ParseUint(_id.(string), 10, 32)
			if err == nil {
				uid = uint(id)
			}
		}
	...
	logger.AppendDebug("GetUserByToken", user)
	return user
}
